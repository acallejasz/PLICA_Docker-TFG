...
curl -u elastic:xxxx -X GET http://172.21.0.1:9200/
...
curl -u elastic:xxxx -X PUT "172.21.0.1:9200/my-index?pretty" -H 'Content-Type: application/json' -d'
{
  "settings": {
    "index": {
...
curl -u elastic:xxxx -H 'Content-Type: application/x-ndjson' -XPOST '172.21.0.1:9200/my-index/doc/_bulk?pretty' --data-binary @shakespeare_6.0.json

echo
echo "Comprobamos que nuestro índice esta listado en la lista de índices"
echo

curl -u elastic:xxxx -X GET "172.21.0.1:9200/_cat/indices?v&pretty"

echo
echo "Creamos un index index-pattern en Kibana para poder visualizar los datos del índice creado en Elasticsearch"
echo

curl -u elastic:xxxx -X POST 172.21.0.1:5601/api/saved_objects/index-pattern/my-index  -H 'kbn-xsrf: true' -H 'Content-Type: application/json' -d '
{
  "attributes": {
    "title": "my-index"
  }
