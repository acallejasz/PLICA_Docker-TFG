...
kafka_id=$(docker ps -q --filter "name=kafka-docker_kafka_2")
...
sudo docker exec -it $kafka_id /opt/kafka/bin/kafka-console-consumer.sh --topic "WF-DATA" --from-beginning --bootstrap-server 192.168.99.100:9092