...
docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -e HOST_IP=$broker_ip --name producer -i -t acallejasz/kafka /opt/kafka/bin/kafka-console-producer.sh --topic=$1 --bootstrap-server=$broker_ip:$port --producer.config /opt/kafka/config/producer_ssl.properties
